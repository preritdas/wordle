...
class Wordle:
    def __init__(self, word, realWords):
        self.word = word
        self.realWords = realWords
    
    def run(self):
        if self.realWords == True or self.realWords == False:
            if len(self.word) != 5 or self.word not in words:
            ...
            for i in range(6): # 6 attempts
                ...
                # Cheating checks
                while guess.lower() not in words or ' ' in guess or len(guess) > len(self.word):
                    if ' ' in guess:
                        print("You can't have multiple words in your guess. Please run again.")
                    elif len(guess) > len(self.word):
                        print(f"You can't guess a word with more than {len(list(self.word))} letters. Please run again.")
